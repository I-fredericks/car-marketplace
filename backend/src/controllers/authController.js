...
<title>${title} — AutoTrustGhana</title></head>
<body style="margin:0;font-family:Arial,Helvetica,sans-serif;background:#F4F6F9;display:flex;justify-content:center;padding:40px 16px;">
  <div style="max-width:440px;background:#fff;border-radius:16px;padding:32px 28px;text-align:center;box-shadow:0 2px 12px rgba(27,42,74,.08);">
    ...
    <p style="color:#1B2A4A;font-weight:bold;font-size:13px;margin-top:24px;">AutoTrustGhana</p>
  </div>
</body></html>`;
...
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">
<title>Reset password — AutoTrustGhana</title></head>
<body style="margin:0;font-family:Arial,Helvetica,sans-serif;background:#F4F6F9;display:flex;justify-content:center;padding:40px 16px;">
  <div style="max-width:440px;width:100%;background:#fff;border-radius:16px;padding:32px 28px;box-shadow:0 2px 12px rgba(27,42,74,.08);">
    ...
    <p style="color:#4B5563;line-height:1.6;margin:0 0 20px;font-size:14px;">AutoTrustGhana — this link expires in 1 hour.</p>
    <div id="form">
...
const verifyEmail = async (req, res) => {
  try {
    ...
    if (record) {
      ...
      return res.send(verifyPage('Email confirmed', 'Your AutoTrustGhana account is now active. Welcome aboard!', true));
    }
  ...
  try {
    ...
    const html = `
      <div style="font-family:Arial,Helvetica,sans-serif;max-width:520px;margin:0 auto;padding:24px;">
        <h2 style="color:#1B2A4A;margin-bottom:16px;">AutoTrustGhana — Staff Portal</h2>
    ...
    await sendMail({
      ...
      text: `Your AutoTrustGhana staff sign-in code is ${code} (valid ${ADMIN_OTP_MINUTES} minutes).`,
      html,
    });
