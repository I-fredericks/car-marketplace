...
//   BANK_TRANSFER / MOMO: buyer pays into SikaRide's collection account ->
...
const sendReceiptEmail = (purchase, vehicle, buyer) => {
  ...
  return sendMail({
    to: buyer.email,
    subject: `Payment confirmed — ${purchase.reference} (SikaRide)`,
    ...
    html: `
      <div style="font-family:Arial,Helvetica,sans-serif;max-width:520px;margin:0 auto;padding:24px;">
        <h2 style="color:#1B2A4A;margin-bottom:8px;">SikaRide</h2>
...
const instructionsFor = (purchase) => ({
  ...
  note: 'Send exactly this amount and put the reference as the transfer reason, then tap "I have paid". SikaRide confirms against the account statement before your money counts as escrowed.',
});
...
const openDispute = async (req, res) => {
  try {
    ...
    if (!reason || String(reason).trim().length < 10) {
      return res.status(400).json({ message: 'Describe the problem in at least 10 characters so SikaRide can mediate.' });
    }
...
const confirmReceived = async (req, res) => {
  try {
    ...
    if (purchase.disputeStatus === 'OPEN') {
      return res.status(400).json({ message: 'A dispute is open on this order — SikaRide resolves it before funds move.' });
    }
...
const cancelPurchase = async (req, res) => {
  try {
    ...
    if (purchase.disputeStatus === 'OPEN') {
      return res.status(400).json({ message: 'A dispute is open on this order — SikaRide resolves it before anything changes.' });
    }
