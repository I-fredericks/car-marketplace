...
const CarDetails = () => {
  ...
  useSEO(car ? {
    ...
    description: `${car.year} ${car.make} ${car.model} — GH₵${Number(car.price).toLocaleString()} in ${car.location}, Ghana. ${car.condition?.replace('_', ' ')} · ${car.mileage ? `${car.mileage.toLocaleString()} km` : 'low mileage'} · ${car.transmission}. Contact the seller on SikaRide.`,
  ...
  const shareListing = async () => {
    const text = `Check out this ${car.year} ${car.make} ${car.model} — GH₵ ${Number(car.price).toLocaleString()} on SikaRide`;
  ...
  // "Verified" means SikaRide reviewed the seller's documents — never just
  // their account role
            ...
            {reviewsData && reviewsData.reviews.length > 0 && (
              <div className="mb-10">
                ...
                <p className="text-xs text-textmuted mt-3 flex items-center gap-1.5">
                  ...
                  Every review is from a buyer who completed an escrowed purchase on SikaRide.
                </p>
              </div>
            )}
          </div>
